...
const TalkToMentor = () => {
	...
	const submitDetails = e => {
		...
		const data = { ...user, timeForCall: time };
		...
		agent.TalkToMentor.sendDetails(data).then(res => {
			...
			setRes(`You will recieve a call from our executive on ${user.dateForCall} between ${time} and ${x}:00`);
			setError("");
		}).catch(err => {
			setLoading(false);
				...
				<div className='book-a-call-div'>
					...
					<form onSubmit={submitDetails}>
						...
						<div className='flex  flex-column'>
							...
							<input required type="number" value={user.phoneNo} onChange={(e) => setUser({ ...user, phoneNo: e.target.value })} />
						</div>
						<div className='flex  flex-column'>
							<label htmlFor="date"> Time for Call: </label>
							<div className='inputs'>
								<input required type="date" min={minDate} value={user.dateForCall} onChange={(e) => setUser({ ...user, dateForCall: e.target.value })} />
